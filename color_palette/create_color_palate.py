...
def two_value_coding(i):
	...
	if i > max_combination or i <= 0:
		raise Exception(f"that number in array cant be without repetition. Should be: 0 < {i} <={max_combination}")
...
def save_json(dict_data, save_name):
